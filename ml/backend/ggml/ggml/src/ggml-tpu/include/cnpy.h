// cnpy.h
// Header-only version of cnpy library
// Supports reading and writing Numpy .npy and .npz files
...
// Begin of cnpy namespace
namespace cnpy {

// Structure to hold Numpy arrays
struct NpyArray {
...
inline void parse_npy_header(FILE* fp,size_t& word_size, char& type,
        std::vector<size_t>& shape, bool& fortran_order);

inline void parse_npy_header(unsigned char* buffer, size_t& word_size, char& type,
        std::vector<size_t>& shape, bool& fortran_order);

inline void parse_zip_footer(FILE* fp, uint16_t& nrecs, size_t& global_header_size,
...
// Operator overloads for appending data to a vector<char>
template<typename T>
...
template<typename T>
inline void npy_save(std::string fname, const std::vector<T> data,
        std::string mode = "w");

template<typename T>
inline void npz_save(std::string zipname, std::string fname,
...
template<typename T>
inline void npz_add_array(npz_t &map, std::string fname,
        const T* data, const std::vector<size_t> shape);
...
inline void npz_save_all(std::string zipname, npz_t &map);

// Internal helper functions
inline static char BigEndianTest();
...
// Implementation of BigEndianTest
inline static char BigEndianTest() {
    int x = 1;
    return (((char *)&x)[0]) ? '<' : '>';
}

// Implementation of create_npy_header
inline std::vector<char> create_npy_header(const std::vector<size_t>& shape,
        size_t word_size, char type) {
...
// Implementation of parse_npy_header from buffer
inline void parse_npy_header(unsigned char* buffer, size_t& word_size,  char& type,
        std::vector<size_t>& shape, bool& fortran_order) {
...
// Implementation of parse_npy_header from FILE*
inline void parse_npy_header(FILE* fp, size_t& word_size, char& type,
        std::vector<size_t>& shape, bool& fortran_order) {
    char buffer[256];
...
// Implementation of parse_zip_footer
inline void parse_zip_footer(FILE* fp, uint16_t& nrecs, size_t& global_header_size,
        size_t& global_header_offset) {
...
// Implementation of npy_save for raw pointers
template<typename T>
inline void npy_save(std::string fname, const T* data,
    ...
    size_t word_size_new = sizeof(T);
    char type_new = mapType<T>::value;
    std::vector<char> header = create_npy_header(true_data_shape, word_size_new, type_new);
...
// Explicit template instantiation declarations for npy_save
...
// Implementation of npy_save for vectors
template<typename T>
inline void npy_save(std::string fname, const std::vector<T> data,
        std::string mode) {
    std::vector<size_t> shape;
...
// Implementation of npz_save for raw pointers
template<typename T>
inline void npz_save(std::string zipname, std::string fname,
        const T* data, const std::vector<size_t>& shape,
        std::string mode) {
    ...
    local_header += (uint16_t) 0x0403; // Second part of signature
    local_header += (uint16_t) 20; // Minimum version to extract
    ...
    local_header += (uint16_t) fname.size(); // Filename length
    ...
    */
    const size_t ZIP64_LIMIT = ((((size_t)1) << 31) - 1);
    if (global_header_offset + nbytes + local_header.size() >= ZIP64_LIMIT) {
      // Build global header
      global_header += "PK"; // First part of signature
      global_header += (uint16_t) 0x0201; // Second part of signature
      global_header += (uint8_t) 45; // Create version
      global_header += (uint8_t) 3; // zinfo.create_system
      global_header += (uint8_t) 45; // Extract version
      global_header += (uint8_t) 0; // zinfo.reserved
      global_header.insert(global_header.end(),local_header.begin()+6,
                           local_header.begin()+28);
      global_header += (uint16_t) 12; // Extra data length
      ...
      // Relative offset of local file header
      // Since it begins where the global header used to begin
      global_header += (uint32_t) (global_header_offset + nbytes + local_header.size());
      global_header += fname;
      fwrite(&global_header[0],sizeof(char),global_header.size(),fp);
    }
    if (global_header_offset >= ZIP64_LIMIT) {
      // ZIP64 End Archive Record and Locator already handled above
    }
    // Build footer
    std::vector<char> footer;
    footer += "PK"; // First part of signature
    footer += (uint16_t) 0x0605; // Second part of signature
    footer += (uint16_t) 0; // Number of this disk
    footer += (uint16_t) 0; // Disk where footer starts
    footer += (uint16_t) (nrecs+1); // Number of records on this disk
    footer += (uint16_t) (nrecs+1); // Total number of records
    footer += (uint32_t) global_header.size(); // Number of bytes of global headers
...
// Explicit template instantiation declarations for npz_save
...
// Implementation of npz_save for vectors
template<typename T>
inline void npz_save(std::string zipname, std::string fname,
...
// Implementation of npz_save for NpyArray
template<typename T>
inline void npz_save(std::string zipname, std::string fname,
...
// Implementation of npz_add_array for raw pointers
template<typename T>
inline void npz_add_array(npz_t &map, std::string fname,
        const T* data, const std::vector<size_t> shape) {
    size_t word_size = sizeof(T);
...
// Implementation of npz_clone_array
inline void npz_clone_array(npz_t &map, std::string fname, std::string new_name) {
    auto array = map[fname];
    map[new_name] = array;
}

// Explicit template instantiation declarations for npz_add_array
...
// Implementation of npz_add_array for vectors
template<typename T>
inline void npz_add_array(npz_t &map, std::string fname,
        const std::vector<T> &data) {
    std::vector<size_t> shape;
...
// Explicit template instantiation declarations for npz_add_array with vectors
template void npz_add_array<std::complex<double> >(npz_t &, std::string,
...
// Implementation of npz_save_all
inline void npz_save_all(std::string zipname, npz_t &map) {
    for (auto it = map.begin(); it != map.end(); it++) {
        ...
        if (arr.type == 'f') {
            ...
            assert(arr.word_size == sizeof(float) ||
                   arr.word_size == sizeof(double) ||
                   arr.word_size == sizeof(long double));
            if(arr.word_size == sizeof(float)) {
                npz_save<float>(zipname, it->first, it->second, mode);
            }
            else if(arr.word_size == sizeof(double)) {
                npz_save<double>(zipname, it->first, it->second, mode);
            }
            else {
...
// Implementation of load_the_npy_file
inline static NpyArray load_the_npy_file(FILE* fp) {
    std::vector<size_t> shape;
    size_t word_size;
    char type;
    bool fortran_order;
...
// Implementation of load_the_npz_array
inline static NpyArray load_the_npz_array(FILE* fp, uint32_t compr_bytes,
...
// Implementation of npz_load (load all arrays)
inline npz_t npz_load(std::string fname) {
    npz_t arrays;
    arrays.clear();
    FILE* fp = fopen(fname.c_str(),"rb");
    if(!fp) {
        // If file can't be opened, return empty map
        return arrays;
    }
    while(1) {
        ...
        if(headerres != 30)
            break;
        // Check if we've reached the global header
        ...
        // Get compression method and sizes
        uint16_t compr_method = *reinterpret_cast<uint16_t*>(&local_header[0]+8);
        uint32_t compr_bytes = *reinterpret_cast<uint32_t*>(&local_header[0]+18);
        uint32_t uncompr_bytes = *reinterpret_cast<uint32_t*>(&local_header[0]+22);
        // Load the array based on compression method
        if(compr_method == 0) {
...
// Overloaded npz_load accepting FILE*
inline npz_t npz_load(FILE* fp) {
    npz_t arrays;
    arrays.clear();
    if(!fp) {
        // If file can't be opened, return empty map
...
// Implementation of npz_load for specific variable
inline NpyArray npz_load(std::string fname, std::string varname) {
    FILE* fp = fopen(fname.c_str(),"rb");
    ...
    while(1) {
        ...
        uint32_t compr_bytes = *reinterpret_cast<uint32_t*>(&local_header[0]+18);
        uint32_t uncompr_bytes = *reinterpret_cast<uint32_t*>(&local_header[0]+22);
        // If variable matches, load and return it
        if(vname == varname) {
    ...
    fclose(fp);
    // If we get here, the variable was not found
    throw std::runtime_error("npz_load: Variable name "+varname+" not found in "+fname);
}

// Implementation of npy_load
inline NpyArray npy_load(std::string fname) {
    FILE* fp = fopen(fname.c_str(), "rb");
